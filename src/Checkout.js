...
function Checkout({ cart, totalPrice, onBack, onOrderComplete }) {
  ...
  const MERCHANT_NAME = "Ruchi Bakers"; // Your business name
  // ============================================
  ...
  const generatePhonePePaymentLink = () => {
    ...
    const transactionNote = `Order from ${formData.name} - Ruchi Bakers`;
  ...
  const handleGPay = () => {
    ...
    const upiId = encodeURIComponent(MERCHANT_UPI_ID);
    const merchantName = encodeURIComponent(MERCHANT_NAME);
    const transactionNote = encodeURIComponent(`Order from ${formData.name} - Ruchi Bakers`);
